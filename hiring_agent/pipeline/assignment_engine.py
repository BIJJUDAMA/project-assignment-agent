...
linear sum assignment (Hungarian algorithm) to maximize overall fit quality.
"""
...
class AssignmentEngine:
    def assign_projects(
        self,
        candidates: List[Tuple[JSONResume, str]],  # List of (resume_data, candidate_id/name)
        ...
        # Each column in our cost matrix represents a specific slot in a project
        slot_to_project = []
        ...
        # Ensure the total slots equals total candidates
        assert len(slot_to_project) == N, f"Mismatch: slots={len(slot_to_project)}, candidates={N}"

        # 3. Construct the cost matrix
        # Matrix size: N candidates (rows) x N project slots (columns)
        cost_matrix = np.zeros((N, N))

        for row_idx, (resume, candidate_name) in enumerate(candidates):
        ...
        for row_idx, col_idx in zip(row_ind, col_ind):
            resume, candidate_name = candidates[row_idx]
